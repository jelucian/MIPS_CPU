...
module MCU(sys_clk, reset, intr, c, n, z, v, IR, int_ack, pc_sel, pc_ld, pc_inc,
           ...
           dm_cs, dm_rd, dm_wr, FS, S_Addr, T_Addr, D_Addr, shamt);
    ...
    always @ (posedge sys_clk, posedge reset)
      if(reset)
        ...
        //#1//wait for flags to update for simulation purposes
        case(state)
          FETCH:
            if(int_ack == 0 & intr == 1)
          ...
          INPUT:
            begin
              ...
              {S_Addr, T_Addr, D_Addr, shamt} = { IR[26:21], IR[20:16], IR[15:11], IR[10:6] };
              state = FETCH; {IE, N, Z, V, C} = {IE, N, Z, V, C};
              $display("T = %t | State = INPUT    | Next State = FETCH  | PC = %h | IR = %h",
                       $time, CPU_IU.PC_out, CPU_IU.IR_out);
            end
          ...
          OUTPUT:
            begin
              ...
              {S_Addr, T_Addr, D_Addr, shamt} = { IR[26:21], IR[20:16], IR[15:11], IR[10:6] };
              state = FETCH; {IE, N, Z, V, C} = {IE, N, Z, V, C};
              $display("T = %t | State = OUTPUT    | Next State = FETCH  | PC = %h | IR = %h",
                       $time, CPU_IU.PC_out, CPU_IU.IR_out);
            end
